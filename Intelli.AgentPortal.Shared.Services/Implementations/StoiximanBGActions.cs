...
{
    public class StoiximanBGActions : ICompanyActions
    {
        public List<CustomerData> AgentCustomerData(Batch batch)
        {
            ...
            if (batch.BatchSourceId == batchSourcesList.FirstOrDefault(d => d.EnumValue == BatchSourcesKeys.StoiximanBG_ID)?.Id || batch.BatchSourceId == batchSourcesList.FirstOrDefault(d => d.EnumValue == BatchSourcesKeys.StoiximanBG_IDnPOA1)?.Id ||
            batch.BatchSourceId == batchSourcesList.FirstOrDefault(d => d.EnumValue == BatchSourcesKeys.StoiximanBG_PoA)?.Id || batch.BatchSourceId == batchSourcesList.FirstOrDefault(d => d.EnumValue == BatchSourcesKeys.StoiximanBG_ID2)?.Id)
            {
